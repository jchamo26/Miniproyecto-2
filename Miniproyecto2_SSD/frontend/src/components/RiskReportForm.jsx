...
export default function RiskReportForm({ riskReport, patientId, onSigned }) {
  ...
  const handleSign = async () => {
    ...
    if (action === 'REJECTED' && rejectionReason.length < 20) {
      toast.error('Justificación de rechazo debe tener al menos 20 caracteres')
      return
    }
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/fhir/RiskAssessment/${riskReport.id}/sign`, {
        method: 'PATCH',
        headers: {
  ...
  return (
    ...
    >
      <h2 className="text-xl font-bold">✍️ Firma del RiskReport</h2>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Ingrese sus observaciones clínicas..."
        ...
        <p className={`text-xs mt-1 ${notesLength >= 30 ? 'text-green-400' : 'text-slate-500'}`}>
          {notesLength}/30 caracteres {notesLength >= 30 && '✅'}
        </p>
      </div>
      ...
      {action === 'REJECTED' && (
        ...
        >
          ...
          <textarea
            ...
            placeholder="Explique por qué rechaza este diagnóstico..."
          ...
          <p className={`text-xs mt-1 ${rejectionLength >= 20 ? 'text-green-400' : 'text-slate-500'}`}>
            {rejectionLength}/20 caracteres {rejectionLength >= 20 && '✅'}
          </p>
        </motion.div>
      )}
      ...
      >
        {loading ? '⏳ Firmando...' : '✍️ Firmar RiskReport'}
      </button>

      {/* Disclaimer */}
      <p className="text-xs text-slate-500 text-center">
        Al firmar, confirma que revisó el diagnóstico de IA y lo valida clínicamente
      </p>
    </motion.div>
  )
}
