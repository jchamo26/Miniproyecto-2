...
ACCESS_KEY = os.getenv("ACCESS_KEY", "test-access-key-12345")
...
async def create_fhir_patient(patient_name, birth_date, gender):
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            print(f"❌ Error creating FHIR Patient: {response.status_code}")
            return None
...
async def seed_patients_from_pima():
    ...
    """
    print("🌱 Seeding patients from PIMA Diabetes dataset...")
    ...
    if csv_path.exists():
        ...
        print("   ⚠️ CSV not found, generating synthetic PIMA-like data...")
        np.random.seed(42)
        n_patients = 30
    ...
    for idx, row in df.iterrows():
        try:
            ...
            print(f"      📈 Risk: {risk_cat} ({risk_score:.2f})")
        
        except Exception as e:
            print(f"   Error processing patient {idx}: {e}")
            continue
    
    print(f"\n✅ Seeded {len(df)} patients successfully")

async def main():
    """Main seed script"""
    print("=" * 60)
    print("🌱 PATIENT SEEDING SCRIPT")
